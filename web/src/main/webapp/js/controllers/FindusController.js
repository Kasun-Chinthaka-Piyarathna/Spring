(function () {
    ...
    function findusController($scope, findusService, $stateParams, $rootScope, $mdDialog, restaurantService) {
        ...
        restaurantService.getAllFood_Items($scope.restaurant.Restaurant_ID).then(function sucess(data) {


            $scope.restaurant.AvailableFoodItems = data;
        ...
        $scope.addToCart = function (item, quantity_new) {
            ...
            if (!($rootScope.allItems == null)) {
                for (var i = 0; i < $rootScope.allItems.length; i++) {
                    if ($rootScope.allItems[i].name == item.name) {
                        $rootScope.allItems[i].selectedQuantity = quantity_new;
                        var success;
                        success = "Done";
                    }
                }
                if (!(success == "Done")) {
                    item.selectedQuantity = quantity_new;
        ...
        $scope.showAdvanced = function (ev, restaurant) {
            ...
            $mdDialog.show({
                ...
                templateUrl: 'views/dialog2.tmpl.jsp',
